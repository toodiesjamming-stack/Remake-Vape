...
do
	...
	if _hookDetected then
		game:GetService('Players').LocalPlayer:Kick('[SKIDV4] integrity check failed - what u trying to do???')
		error('[SKIDV4] loadstring hook detected', 2)
	end
end
...
local function finishLoading()
	...
	vape:Clean(playersService.LocalPlayer.OnTeleport:Connect(function()
		if (not teleportedServers) and (not shared.VapeIndependent) then
			...
			local teleportScript = [[
				loadstring(game:HttpGet('https://raw.githubusercontent.com/toodiesjamming-stack/Remake-Vape/'..readfile('newvape/profiles/commit.txt')..'/loader.lua', true), 'loader')()
			]]
			...
			if not _ok then warn('[SKIDV4] save failed before teleport: ' .. tostring(_err)) toclipboard(_err) end
	...
	if not shared.vapereload then
		...
		if vape.Categories.Main.Options['GUI bind indicator'].Enabled then
			...
			vape:CreateNotification('[SKIDV4] Finished Loading', name .. (vape.VapeButton and 'Press the button in the top right to open GUI' or 'Press ' .. table.concat(vape.Keybind, ' + '):upper() .. ' to open GUI'), 5)
		end
	end
end
...
if not shared.VapeIndependent then
	...
	if isfile('newvape/games/' .. gameFileId .. '.lua') then
		...
		if not shared.VapeDeveloper then
			local suc, res = pcall(function()
				return game:HttpGet('https://raw.githubusercontent.com/toodiesjamming-stack/Remake-Vape/' .. readfile('newvape/profiles/commit.txt') .. '/games/' .. gameFileId .. '.lua', true)
			end)
